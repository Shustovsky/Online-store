...
export default function createCatalog(products: Product[]): void {
    ...
    const filters = createFilters(products);
    ...
    const productsSort = createSortBlock();
    ...
    products.forEach((item) => productsItems.append(createItems(item)));

    changeView();
}

function createFilters(products: Product[]): HTMLDivElement {
    ...
    const btnCopy = createBtn('Copy');
    filtersButtons.append(btnReset);
    filtersButtons.append(btnCopy);

    const categoryFilter = createCheckboxFilter('category', products);
    const brandFilter = createCheckboxFilter('brand', products);
    filters.append(categoryFilter);
    filters.append(brandFilter);

    const priceFilter = createRangeFilter('price');
    const stockFilter = createRangeFilter('stock');
    filters.append(priceFilter);
    filters.append(stockFilter);

    setValueRangeFilter(priceFilter);
    setValueRangeFilter(stockFilter);

    return filters;
}

function createBtn(name: string): HTMLButtonElement {
...
function createCheckboxFilter(name: string, products: Product[]): HTMLDivElement {
...
function createRangeFilter(name: string): HTMLDivElement {
    const filtersCategory = document.createElement('div');
    filtersCategory.className = `filters__${name}`;
...
function setValueRangeFilter(div: HTMLDivElement): void {
...
function createSortBlock() {
    ...
    const sortTitle = createOption('sort-title', 'Sort options:');
    const priceHigh = createOption('price-high', 'Price: High to Low');
    const priceLow = createOption('price-low', 'Price: Low to High');
    const ratingA = createOption('rating-a', 'Name: A to Z');
    const ratingZ = createOption('rating-z', 'Name: Z to A');
...
function createOption(value: string, inner: string): HTMLOptionElement {
...
function createItems(product: Product): HTMLDivElement {
