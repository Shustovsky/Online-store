...
export function drawShoppingCart(shoppingCart: ShoppingCart): void {
    ...
    shoppingCart.products.forEach((value, index) => {
        const contents = createProduct(value.product, value.count, index + 1);
        shoppingcartContents.append(contents);
    });
    container.append(shoppingcartContents);

    const summaryWrapper = createSummaryWrapper(shoppingCart);
    container.append(summaryWrapper);
}

function createProduct(product: Product, productsCount: number, productSequenceNumber: number): HTMLDivElement {
    const shoppingcartProductWrapper = document.createElement('div');
    shoppingcartProductWrapper.className = 'shoppingcart__product';

    const productDisplay = createProductDisplay(product, productSequenceNumber);
    ...
    const changeProductQuantity = createChangeProductQuantity(product, productsCount);
    shoppingcartProductWrapper.append(changeProductQuantity);

    return shoppingcartProductWrapper;
}

function createTitleWrapper(): HTMLDivElement {
    ...
    const shoppingcartPaginationCount = createPaginationCount();
    titleWrapper.append(shoppingcartPaginationCount);

    const shoppingcartPaginationPage = createPaginationPage();
    titleWrapper.append(shoppingcartPaginationPage);

    return titleWrapper;
}

function createPaginationPage(): HTMLDivElement {
...
function createPaginationCount(): HTMLDivElement {
...
function createProductDisplay(product: Product, index: number): HTMLDivElement {
...
function createProductDescription(product: Product): HTMLDivElement {
...
function createProductDetails(product: Product): HTMLDivElement {
    const productDetails = document.createElement('div');
    productDetails.className = 'shoppingcart__wrapper-details';

    const rating = createRatingWrapper(product);
    productDetails.append(rating);

    const productDiscount = createProductDiscount(product);
    productDetails.append(productDiscount);

    return productDetails;
}

function createRatingWrapper(product: Product): HTMLDivElement {
...
function createProductDiscount(product: Product): HTMLDivElement {
...
function createChangeProductQuantity(product: Product, productsCount: number): HTMLDivElement {
    const changeProductQuantity = document.createElement('div');
    changeProductQuantity.className = 'shoppingcart__change-quantity';

    const productStockWrapper = createProductStockWrapper(product);
    changeProductQuantity.append(productStockWrapper);

    const productAddWrapper = createProductAddWrapper(productsCount);
...
function createProductStockWrapper(product: Product): HTMLDivElement {
...
function createProductAddWrapper(productCount: number): HTMLDivElement {
    const productAddWrapper = document.createElement('div');
...
function createSummaryWrapper(shoppingCart: ShoppingCart): HTMLDivElement {
    ...
    const summary = createSummary(shoppingCart);
    summaryWrapper.append(summary);

    return summaryWrapper;
}

function createSummary(shoppingCart: ShoppingCart): HTMLDivElement {
    const displayCart = document.createElement('div');
    displayCart.className = 'shoppingcart__display-cart';

    const productsWrapper = createProductTotalCountWrapper(shoppingCart);
    displayCart.append(productsWrapper);

    const totalWrapper = createProductsTotalPrice(shoppingCart);
...
function createProductTotalCountWrapper(shoppingCart: ShoppingCart): HTMLDivElement {
...
function createProductsTotalPrice(shoppingCart: ShoppingCart): HTMLDivElement {
